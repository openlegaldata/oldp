...
class CourtListView(SortableFilterView):
    ...
    columns = [
        SortableColumn(label=_("Court title"), field_name="name", sortable=True),
        SortableColumn(
            label=_("ECLI code"),
            field_name="ecli",
    ...
    def get_queryset(self):
        return (
            ...
            .order_by("name")
            .defer(*Court.defer_fields_list_view)
        )
...
class CourtCasesListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"nav": "courts", "title": self.court.name, "court": self.court})
        return context
