...
class Country(models.Model):
    ...
    code = models.CharField(
        max_length=2, help_text="ISO country code (en, de, fr, ...)"
    )
...
class State(models.Model):
    ...
    slug = models.SlugField(unique=True, max_length=50, help_text="Name field as slug")
...
class City(models.Model):
    ...
    state = models.ForeignKey(
        State, on_delete=models.CASCADE, help_text="State of city"
    )
...
class Court(models.Model):
    ...
    aliases = models.TextField(
        null=True, blank=True, help_text="List of aliases (one per line)"
    )
    ...
    slug = models.SlugField(
        unique=True, max_length=60, help_text="Type & city name as lowercase"
    )
    ...
    homepage = models.URLField(
        blank=True, null=True, help_text="Official court homepage"
    )
    ...
    postal_code = models.CharField(
        max_length=200, null=True, blank=True, help_text="Postal code (ZIP code)"
    )
    address_locality = models.CharField(
        max_length=200, null=True, blank=True, help_text="Locality (city name)"
    )
    telephone = models.CharField(
        max_length=200, null=True, blank=True, help_text="Telephone number"
    )
    fax_number = models.CharField(
        max_length=200, null=True, blank=True, help_text="Fax number"
    )
    email = models.EmailField(
        max_length=200, null=True, blank=True, help_text="Email address"
    )
...
def pre_save_court(sender, instance: Court, *args, **kwargs):
    if instance.slug is None or instance.slug == "":
        if instance.court_type is not None and instance.city is not None:
            instance.slug = (
                instance.court_type.lower() + "-" + slugify(instance.city.name)
            )
        else:
