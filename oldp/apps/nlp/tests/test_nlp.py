...
class NLPTestCase(TestCase):
    ...
    def test_spacy_tokenization(self):
        ...
        self.assertEqual(expected_tokens, list(container.tokens()))
    ...
    def test_nltk_tokenization(self):
        ...
        container = nlp.process(text)
        self.assertEqual(expected_tokens, container.tokens())
    ...
    def test_spacy_lemmatization(self):
        ...
        expected_lemmas = ['In', 'der', 'USA', 'kosten', 'einen', 'Burger', '€', '5', ',',
                           'beziehungsweise', '5', '€']
        container = nlp.process(text)
        self.assertEqual(expected_lemmas, list(container.lemmas()))
    ...
    def test_spacy_entities(self):
        ...
        self.assertEqual([('Alba', 72, 76),
                         ('Italien', 78, 85)],
                         list(container.ents(Entity.LOCATION)))
        self.assertEqual([('Ferrero', 10, 17)],
                         list(container.ents(Entity.ORGANIZATION)))
        self.assertEqual([('Pietro Ferrero', 33, 47)],
                         list(container.ents(Entity.PERSON)))
