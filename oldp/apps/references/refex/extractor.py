...
from oldp.apps.references.refex.errors import RefExError, AmbiguousReferenceError
from oldp.apps.references.refex.models import RefMarker, Ref, RefType
...
class RefExtractor(object):
    ...
    def extract(self, content_html: str):
        ...
        if self.do_law_refs:
            content, refs = self.extract_law_ref_markers(content, key=len(references))
            reference_markers.extend(refs)

            logger.debug('Extracted law refs: %i' % len(refs))

        if self.do_case_refs:
            content, refs = self.extract_case_ref_markers(content, key=len(references))
            reference_markers.extend(refs)
    ...
    def get_law_book_codes(self):
        if self.law_book_codes is None:
            ...
            self.law_book_codes.extend(['AsylG', 'VwGO', 'GkG', 'stbstg', 'lbo', 'ZPO', 'LVwG', 'AGVwGO SH', 'BauGB',
                                       'BauNVO', 'ZWStS', 'SbStG', 'StPO', 'TKG'])
            raise ValueError('law book codes not set!')
    ...
    def extract_law_ref_markers(self, content: str, key: int=0) -> Tuple[str, List[RefMarker]]:
        """
        § 3d AsylG
        § 123 VwGO
        §§ 3, 3b AsylG
        ...
        for ref_m in results:
            ...
            content, marker_offset = marker.replace_content(content, marker_offset, key + len(markers))
    ...
    def extract_case_ref_markers(self, content: str, key: int=0) -> Tuple[str, List[RefMarker]]:
        """
            ...
            for m in matches:
                ...
                for focus in focus_all:
                    ...
                    if len(fns_matches) == 1:
                        ...
                        content, marker_offset = marker.replace_content(content, marker_offset, key + len(refs))
