"""Tests for the law and lawbook creation admin dashboards."""
...
class LawCreationDashboardTestCase(TestCase):
    """Tests for the law creation dashboard view."""
    ...
    def setUp(self):
        ...
        self.url = reverse("laws_creation_dashboard")
    ...
    def _create_law(self, slug, days_ago=0, token=None, review_status="accepted"):
        """Helper to create a law with a specific created_date offset."""
        ...
        if days_ago:
            ...
            Law.objects.filter(pk=law.pk).update(created_date=target_date)
            law.refresh_from_db()
        return law
    ...
    def test_dashboard_token_filter_none(self):
        """Test filtering for non-API laws (no token)."""
        self.client.login(username="staffuser", password="testpass123")
        token = APIToken.objects.create(user=self.staff_user, name="Test Token")
        self._create_law("law-with-token", token=token)
        self._create_law("law-without-token")
    ...
    def test_dashboard_token_filter_specific(self):
        ...
        self._create_law("law-token1", token=token1)
        self._create_law("law-token2", token=token2)
    ...
    def test_dashboard_batch_processing_post(self):
        """Test batch processing changes law review status."""
        self.client.login(username="staffuser", password="testpass123")
        law = self._create_law("batch-test-law", review_status="accepted")
        ...
        law.refresh_from_db()
        self.assertEqual(law.review_status, "pending")
...
class LawBookCreationDashboardTestCase(TestCase):
    """Tests for the lawbook creation dashboard view."""
    ...
    def setUp(self):
        ...
        self.url = reverse("lawbooks_creation_dashboard")
        self._book_counter = 0

    def _create_lawbook(self, days_ago=0, token=None, review_status="accepted"):
        """Helper to create a lawbook with a specific created_date offset."""
        self._book_counter += 1
        book = LawBook.objects.create(
            title=f"Test Book {self._book_counter}",
            code=f"TB{self._book_counter}",
            slug=f"test-book-{self._book_counter}",
            created_by_token=token,
            review_status=review_status,
        )
        if days_ago:
            target_date = timezone.now() - timedelta(days=days_ago)
            LawBook.objects.filter(pk=book.pk).update(created_date=target_date)
    ...
    def test_dashboard_token_filter_none(self):
        """Test filtering for non-API lawbooks (no token)."""
        self.client.login(username="staffuser", password="testpass123")
        token = APIToken.objects.create(user=self.staff_user, name="Test Token")
        self._create_lawbook(token=token)
        self._create_lawbook()
    ...
    def test_dashboard_token_filter_specific(self):
        ...
        token1 = APIToken.objects.create(user=self.staff_user, name="Token 1")
        token2 = APIToken.objects.create(user=self.staff_user, name="Token 2")
        self._create_lawbook(token=token1)
        self._create_lawbook(token=token2)
    ...
    def test_dashboard_batch_processing_post(self):
        """Test batch processing changes lawbook review status."""
        self.client.login(username="staffuser", password="testpass123")
        book = self._create_lawbook(review_status="accepted")
        ...
        response = self.client.post(
            ...
            {
                "selected_dates": [today_str],
                "processing_step": "set_lawbook_review_pending",
                "token": "",
            },
        )
        self.assertEqual(response.status_code, 302)
        ...
        book.refresh_from_db()
        self.assertEqual(book.review_status, "pending")
